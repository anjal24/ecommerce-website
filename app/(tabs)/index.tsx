...
const [products, setProducts] = useState<Product[]>([]) // state product type
const [loading, setLoading] = useState(true) //one more state i.e loading


  const categories= [{id: 'all' ,name:'All',icon: "grid"}, ...CATEGORIES]
//we have to  load the product from assets , for that will create the the functions
...
<ScrollView horizontal showsHorizontalScrollIndicator={false}>{categories.map((cat:any)=>(
...
<Text className='text-secondary text-sm'>See All</Text>
  ...
  (
   <View className='flex-row flex-wrap justify-between'>{products.slice(0,4).map((product)=>(
